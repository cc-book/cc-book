...
The TEE sends evidence directly to the Verifier; the Verifier returns the result to the Relying Party. This is the model CoCo uses, with lazy attestation triggered when secrets are needed.
...
The TEE receives an attestation token/passport from the Verifier and presents it directly to Relying Parties.
...
| Model | Flow | Best For |
|---|---|---|
| **Background Check** | TEE → Evidence → Verifier → Result → Relying Party | Lazy attestation on resource request |
